...
def one_dimensional (X):
    ...
    """ 
    One-Dimensional Test Function. cf. Eq.(16)
    """
    return np.square((6 * X) - 2) * np.sin((12 * X) - 4) + (8 * X) 


def three_dimensional (X):
    
    """ 
    Three-Dimensional Test Function. cf. Eq.(17)
    """
    x1, x2, x3 = X.ravel()
    problem = Branin(ndim = 2)
    return problem(np.asarray([[x1],[x2]]).reshape(1,2))[0][0] + one_dimensional(x3)
...
def worst_case_prediction(Point , Model, Scaler):
    ...
    bounds = ((-0.75,0.75),(-0.75,0.75),(-0.05,0.05))
    n_restarts = 2
    n_params = 3
...
def expected_improvement(candidate, robust_optimum, Model, Scaler, n_params = 3):
...
def sample_next_point(acquisition_func, Model, robust_optimum, Scaler, bounds = ((-4.25,9.25),(0.75,14.25),(0.05,0.95)) , n_restarts = 25):
...
def find_robust_optimum (Model, Scaler):
    ...
    bounds = ((-4.25,9.25),(0.75,14.25),(0.05,0.95))
    n_params = 3
    n_restarts = 2
